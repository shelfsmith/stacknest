...
タグ付きリリース以前の開発履歴をフェーズ単位で記載します（古い順）。各フェーズの詳細設計は計画リポジトリのスペックにあり、本リポジトリの README「ロードマップ」が要約です。

### Phase 2.1 — 互換インポータ（最初の土台）
...
- **2.5e** 拡張子別ヘルパーマッピング／スライドショー・Zip ヘルパー個別指定／フォルダを外部ヘルパーへ渡す。
- **2.5f** 「最近開いたライブラリ」更新経路の集約。
- **2.5g** bookType 自動分類の設定化（新規取込のみ・閾値設定・手動リラベル併用）。
- **2.5h** 表紙の crop UI（横長カバー対応・縦長正規化）＋表紙圧縮（保存時ダウンサンプル・全表紙再生成）。
- **2.5i** PDF 表紙取込・PDF ページ数認識（zip 内 PDF 含む）、単独 image/PDF/movie/text の追加。
- **2.5k** グリッド／リストのキーボードナビゲーション（矢印・範囲拡大・Enter で開く・Home/End/PageUp/Down 統一）。

### Phase 2.6 — スマートシェルフ・内蔵ビューワ・ウィザード・保全・リブランド
- **2.6a** スマートシェルフ MVP（条件式コレクション）。
- **2.6b** 内蔵ビューワ コア（専用ウィンドウ・全画面・フィット／ズーム／パン・ページ送り・PDF／アーカイブ／フォルダ／単一画像）。
- **2.6b-2** 見開き表示・本ごとページ方向・スライドショー自動進行・続きから読む・巻末挙動（次の巻／ループ）・全画面で開く・HEIC/HEIF/TIFF/AVIF 対応。
- **2.6c** 初回起動ウィザード（内蔵／外部ビューワ選択・内蔵初期設定・最初のライブラリ作成・設定から再表示）。
- **2.6d** マルチ Mac 同時オープン防止のロックファイル＋旧称クリーンアップ＋アプリ内 Help／About。
- **2.6e** リブランド／識別子整理（`app.shelfsmith.stacknest`）と GitHub 移管（`shelfsmith`）。
- **2.6f** README（日英）／CONTRIBUTING 整備、Help メニューを実装内ヘルプページ化。
- **2.6g** Keychain 非依存の生体認証ロック（armedHash 方式・解錠経路から Keychain アクセスを排除）。
