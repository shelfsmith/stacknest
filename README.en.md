...
By default, books open in the **built-in viewer**; the built-in / external choice is kept **separately for images and EPUB** (**Settings ▸ 表示 (Display) ▸ ビューア (Viewer)**). Video and text (txt/md/rtf) can't be opened in the built-in viewer, so they always open externally. To use an external viewer, configure it in **Settings ▸ 表示 (Display) ▸ 外部ビューア (External Viewer)** (`⌘,`). Click "Choose…" and pick an image-viewer app (cooViewer, Avian, Preview, etc.). You can assign a different app per book type (image / text / video), and EPUB can have its own app on top of that (falling back to the "text" setting when unset). Without this, the system's Archive Utility would just extract `.zip` files instead of viewing them. The selected viewer is persisted across launches via `UserDefaults`.
