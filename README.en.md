...
- **Viewer key rebinding**: Settings ▸ Built-in viewer lets you remap every built-in viewer action to any key (conflicts are rejected; per-row / global reset; the help table reflects the current bindings)
...
- **Built-in viewer**: dedicated window / full-screen viewing with a unified pipeline for zip / cbz / cbr / 7z, folders, single images, and PDF. Fit-to-window, pinch / ± zoom, drag panning, zone-click / arrow / Space paging, and digit-key position jumps. **Two-page spread** (per-book override), per-book page direction (right-to-left / left-to-right), slideshow, resume reading, end-of-book behavior (stop / next volume / loop), previous/next-volume nav, an open-in-full-screen setting, **page-turn effects** (none / fade / slide, shared with EPUB), fully remappable keys (Settings ▸ Built-in viewer), and more. **Instance management** prevents duplicate windows for the same book (opt into a separate window per book via the "Allow multiple viewer windows" setting). Built-in vs. external viewer is switchable in settings (separately for images and EPUB). **Page rendering is tuned per CPU architecture** (full-resolution lazy decoding on Apple Silicon, background downscaling on Intel), and archives are read forward in a single pass, so books with many pages turn without stalling
- **Read EPUBs (beta)**: text EPUBs open on the Mac in a dedicated window (vertical text, ruby, spreads, Home/End for book start/end, font scale `+`/`-`/`=`, saved position, keys shared with the built-in viewer, system / light / dark colour theme). The window shows **whole-book progress ("N / M" and a bar)** and asks **"resume / start over"** for a partly read book. **Volume turns swap the book inside the same window**, and switch between the image viewer and the EPUB window when the format changes (full screen follows the destination's setting). EPUBs also open in the web reader and from remote libraries (StackNest on another Mac); the reading position is shared across all three (the web reader also shows single-image pages full screen and offers resume / start over). All-image EPUBs open in the image viewer. Binding direction is taken from the EPUB at import. **Beta**: illustrations in text books do not form spreads, no search or table of contents, some EPUBs lay out oddly
- **Cover editing**: set a book's cover by **selecting any page inside the archive (EPUB and PDF included)** and **cropping the visible region**, or by **dropping an external image file onto the detail-pane cover** (or the "Set External Image as Cover…" menu → crop). This changes **only the thumbnail (appearance); the archive and the original files are never modified**. Works both locally and remotely (with an editing token), and **when a cover changes on the server, remote clients pick it up on list reload / reconnect** (server-tracking cover cache). **Covers can also be regenerated one book at a time**, and relinking a missing file refreshes the cover and page count automatically
...
- **Watch folders (auto-import)**: watch folders and **auto-import** archives / image folders dropped into them (per-folder naming presets, first-run preview, import summary banner). Auto-classification (bookType), thickness threshold, and whether to prefer the EPUB title can each be **overridden per library** ("follow the StackNest default" / custom). Subfolders are handled per watch folder with **three modes** (ignore / one book per subfolder / recurse into them)
...
- **Offline download**: download selected remote books to local storage and browse / read them **without a connection** (resume supported). Formats set to the external viewer open there (video included, handed over under the book's title). Open from the title screen / File menu "Offline". **Multi-select for batch download / delete** (or "Remove from offline" in the remote library's grid / list context menu)
...
By default, books open in the **built-in viewer**; the built-in / external choice is kept **separately for images and EPUB** (**Settings ▸ 表示 (Display) ▸ ビューア (Viewer)**). Video and text (txt/md/rtf) can't be opened in the built-in viewer, so they always open externally. To use an external viewer, configure it in **Settings ▸ 表示 (Display) ▸ 外部ビューア (External Viewer)** (`⌘,`). Click "Choose…" and pick an image-viewer app (cooViewer, Avian, Preview, etc.). You can assign a different app per book type (image / text / video), and EPUB can have its own app on top of that (falling back to the default setting when unset). Without this, the system's Archive Utility would just extract `.zip` files instead of viewing them. The selected viewer is persisted across launches via `UserDefaults`.

Once configured, double-clicking a book in the grid opens the file (or its cover image) directly in the chosen viewer. Offline (downloaded) books follow the same settings, and the external viewer receives the file under the book's title.
...
**From a web browser**
- Open the share URL on the connecting side to browse with list / grid, full-text search, sort, and paging; open a book to read in the web reader (prefetch, spread / single-page, resume, page-direction sync). Text EPUBs can be read too: single-image pages are shown full screen, and the book's detail sheet offers resume / start over.
...
- From the title screen / File menu **"Offline (downloaded)"**, browse and read downloaded books **even without a server connection** (resume and volume navigation supported; formats set to the external viewer open there). Remove unneeded books via multi-select mode (or "Remove from offline" in the remote library's grid / list context menu).
...
| 4.8 | **EPUB keys unified with the built-in viewer** (same binding table and rebinding; `Home` / `End` reach the book's ends; percentage jumps, spread toggle, text size, full screen, auto-advance, volume navigation; a separate EPUB full-screen setting), **offline copies saved under their original extension** (existing files renamed at launch), Washi 1.17.0 — v0.14.4 | ✅ Done |
| 4.9 | **Easier EPUB reading** (whole-book progress "N / M" and bar, resume / start over, volume turns swapped in the same window, switching with the image viewer, colour theme), **page-turn effects** (none / fade / slide, shared by both viewers), **a separate EPUB external viewer** and external viewers for offline books, a "Built-in viewer" settings tab, more ways into cover editing (EPUB, PDF, cropping a video scene) and an option to use the EPUB title at import, text EPUBs in the web reader (single-image pages full screen, resume / start over), Washi 1.21.0 — v0.15.0 | ✅ Done |
