...
@Suite("updateBookContentHash")
struct UpdateContentHashTests {
    private func freshDB() throws -> Database {
        let dir = FileManager.default.temporaryDirectory.appendingPathComponent("upch_\(UUID().uuidString)")
    ...
    @Test func roundTrip() throws {
        ...
        try db.updateBookContentHash(id: 1, hash: "abc123", size: 4096, mtime: 1_700_000_000.0)
        let book = try #require(try db.fetchAllBooks().first { $0.id == 1 })
        #expect(book.contentHash == "abc123")
        #expect(book.fileSize == 4096)
        #expect(book.fileMtime == 1_700_000_000.0)
    }
}
