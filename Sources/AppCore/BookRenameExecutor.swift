...
public enum BookRenameExecutor {
    ...
    /// 大文字小文字だけが違う移動を、一時名を経由して行う。
    ///
    /// **★ 前提の訂正（2026-09-01 実測）**: 当初この関数は
    /// 「macOS 既定のファイルシステムでは直接 move が『既に存在する』で落ちる」という前提で書かれたが、
    /// **それは誤りだった**。APFS（起動ディスク）でも HFS+（`/Volumes/comic` 等）でも、
    /// `moveItem(abc.zip → ABC.zip)` は**そのまま成功する**。
    /// 誤解の元は `fileExists("ABC.zip") == true` になること —— これは同じファイルを指しているだけで、
    /// move が失敗することを意味しない。**プランナー側で衝突から除外する必要があるのはこのため**であり、
    /// そちらの例外は今も必要。
    ///
    /// **それでもこの関数を残しているのは防御のため**。大文字小文字だけの改名を拒む
    /// ファイルシステム（一部のネットワークマウント等）で動かす可能性があり、
    /// 一時名を挟めばそこでも通る。手元の環境では実質 no-op の遠回りになる。
    ///
