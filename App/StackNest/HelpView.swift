        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 22) {
                ...
                section("ファイル操作・メタデータ") {
                    para("本のファイルやフォルダは、Finder からグリッドへドロップするか、ツールバーの「＋」で追加します。**手動シェルフかお気に入りを表示中なら、そのシェルフにも入ります**（既にライブラリにある本も入ります）。")
                    keyRow("ライブラリから削除", "⌫")
                    keyRow("ゴミ箱へ移動", "⌘⌫")
                ...
                section("表紙を変える") {
                    ...
                    bullet("表紙を編集 — アーカイブ内の任意ページを選び、表示範囲を切り抜き（ページの概念がある形式のみ。**EPUB・PDF も対象**。動画では選べません）")
                    bullet("シーンから表紙を選択… — mp4 / mov / m4v で、シークバー付きのプレイヤーから表紙にするシーンを選ぶ。続けてそのシーンを切り抜くこともでき、切り抜かずに「決定」を押せばシーン全体が表紙になります。選んだシーンは記録されるので、「表紙を再生成」で**同じシーン**を作り直せます（ローカルの本のみ。リモートでは選べません）")
                    bullet("外部画像を表紙に設定… — 任意の画像ファイルを選ぶ（または詳細ペインの表紙へ画像を **ドラッグ&ドロップ**）→ 切り抜いて設定")
                ...
                section("内蔵ビューア") {
                    ...
                    viewerKeyTable
                    para("見開き、ページ方向、続きから読む、巻末挙動は本ごとに永続化されます。内蔵 / 外部ビューアの切替は**画像と EPUB で別々**に持てます（設定 ▸ 表示 ▸ ビューア）。動画とテキスト（txt・md・rtf）は内蔵ビューアで開けないため常に外部ビューアで開きます。EPUB の配色は 設定 ▸ 内蔵ビューア の「EPUB の配色」で「システムに合わせる / ライト / ダーク」から選べます。")
                    para("**ページ送りの演出**は 設定 ▸ 内蔵ビューア の「ページ送りの演出」で「なし / フェード / スライド」から選べます（既定は「なし」）。画像ビューアと EPUB の窓の両方に効きます。隣のページへの送りとスライドショーに掛かり、ジャンプや巻送りには掛かりません。キーを押し続けているときと、システム設定の「視差効果を減らす」が有効なときは省きます。")
                    para("**ルーペ**はカーソル位置を拡大して見せます。ON の間は**スクロールで倍率**が変わり（現在倍率は HUD に出ます）、**形（円 / 正方形）と大きさ（小 / 中 / 大）**は 設定 ▸ 内蔵ビューア で選べ、倍率もそこで確認・既定に戻せます。拡大部分はその場で再デコードするので、拡大しても粗くなりません。")
                    para("**「複数ビューアの起動を許可」**（設定 ▸ 内蔵ビューア）は既定はオフで、同じ本を二重に開かず、別の本を開くと**1つのビューアに保たれます**（開いている本を再度開くと前面化）。オンにすると**別の本は別ウィンドウで開けます**（どちらでも同じ本は1つに集約されます）。")
                }

                section("EPUB を読む（ベータ）") {
                    para("EPUB を取り込むと表紙とメタデータ（題名・著者・言語・綴じ方向）が EPUB から入ります。ダブルクリックで開けます。**ベータ**です: 読めますが、下記の制約の範囲で挙動が固まっていません。")
                    para("EPUB の窓の操作キーは**内蔵ビューアと共通**です（設定 ▸ 内蔵ビューア ▸ キー割り当て）。ページ送り・Home/End（本の最初/最後）・数字キーの位置ジャンプ・`d` 見開き切替・`+`/`-`/`=` 文字倍率・`s` 自動送り・`[`/`]` 巻移動・`f` 全画面・`?` ヘルプが効きます。ルーペやページスキップなど画像専用のものは EPUB では効きません。「全画面で開く（EPUB）」は画像ビューアとは別の設定です。")
                    bullet("**テキストの EPUB** — 専用の窓で縦組み・ルビ・見開き・右綴じ。キー操作は内蔵ビューアと共通の表に従います（Home / End で本の最初 / 最後のページへ、文字倍率は `+` / `-` / `=` で設定として保存）。読書位置は本ごとに保存されます。")
                    bullet("**本全体の進捗** — 窓の下端に本全体の「N / M」と進捗バーを出します（開いた直後やページ送り・マウスの移動で出て、少しすると消えます）。本全体のページ数を測り終えるまでは「計測中…」と出し、バーは章の位置からの概算です。計測中に数字キーで割合ジャンプすると章単位で移動し、「計測中のため章単位で移動」と知らせます。各ページの下に章ごとのページ番号（ノンブル）を出したいときは 設定 ▸ 内蔵ビューア の「EPUB のノンブルを表示」をオンにします。")
                    bullet("**続きから / 最初から** — 途中まで読んだ本を開くと「続きから読みますか？」と訊きます。本の先頭のままなら訊きません。")
                    bullet("**巻送り** — 次の巻（前の巻）がテキストの EPUB なら、**同じ窓の中で本を差し替えます**。読みかけの巻なら「続きから / 最初から」を訊きます。次の巻が画像の本なら画像ビューアへ、画像ビューアから進んだ先がテキストの EPUB なら EPUB の窓へ切り替わります。切り替えたときの全画面は**切り替え先の設定**（「全画面で開く（画像）」/「全画面で開く（EPUB）」）に従います。次の巻を開けないときは窓を閉じずに「次の巻を開けません」と出し、今の本を読み続けられます。")
                    bullet("**窓の位置と大きさ** — EPUB の窓は位置と大きさを覚え、次に開くと同じ位置・大きさで開きます。")
                    ...
                    bullet("**リモート / Web / オフライン** — Web リーダー（ブラウザ・iPad / iPhone）とリモート書庫（別 Mac の StackNest）でも読め、読書位置は Mac・Web・リモートで共有されます。ダウンロード済みの EPUB はオフラインでも読めます（オフラインの位置はサーバとは同期しません）。Web リーダーでは、画像 1 枚だけのページ（表紙・挿絵）を余白なしで画面いっぱいに表示し、本の詳細で読みかけなら「続きから読む / 最初から」を選べます。上下のバーは画面の中央をタップすると出し入れできます。")
                ...
                section("外部ビューア") {
                    para("外部ビューアを使う場合は 設定 ▸ 表示（⌘,）でビューアを選びます。画像と EPUB は別々に選べます。アプリの指定は同じタブの「外部ビューア」で「選択…」からアプリ（cooViewer、Avian、プレビュー 等）を指定します。設定後はグリッドで本をダブルクリックすると外部ビューアで開きます。種類ごとに別のビューアを割り当てることもできます。EPUB は「電子書籍」の行で別のアプリを指定できます（未設定のときは「デフォルト」の設定で開きます）。動画とテキストは内蔵ビューアで開けないため、常に外部ビューアで開きます。オフライン（ダウンロード済み）の本も同じ設定に従い、外部ビューアには本の題名のファイル名で渡します。")
                }
                ...
                section("リモートで使う（共有・クライアント・オフライン）") {
                    ...
                    bullet("ネイティブクライアント — タイトル画面（または File メニュー）の「サーバに接続…」で URL とトークンを入力。サイドバー / ファセット / フィルタ / 詳細のフルブラウズと内蔵ビューアで閲覧、読書進捗はサーバへ同期。**編集権限の共有トークンなら詳細ペインで編集**できます（メタデータの単一 / 複数一括編集、スタンプ付与 / 定義編集、表紙編集〔アーカイブ内ページ選択 / 切り抜き / 外部画像を D&D・メニューで表紙に〕、読み方向。閲覧のみのトークンは読み取り専用）。")
                    bullet("オフライン — 接続中に本を右クリック →「ダウンロード」（「選択」モードで複数選択 → 一括ダウンロードも可）。タイトル画面 / File メニューの「オフライン（ダウンロード済み）」から、サーバに接続していなくても一覧・閲覧（続きから・巻送り）。外部ビューアを選んでいる形式は外部ビューアで開きます。不要分は「選択」モードで一括削除するか、リモート庫のグリッド / 一覧の右クリック「オフラインから削除」で消せます。")
                ...
                section("監視フォルダで自動取込") {
                    para("指定したフォルダを監視し、そこに置かれたアーカイブ / 画像フォルダを**自動でライブラリへ取り込み**ます。設定 ▸ 取り込み（監視フォルダ）でフォルダを追加し、フォルダごとに命名プリセットを割り当てられます。初回はプレビューで確認でき、取込結果はライブラリ窓の上端にバナーで表示されます（**取り込めなかったファイルがあるときは自動で消えず**、「詳細」からどのファイルがなぜ失敗したかを確認できます。× で閉じられます）。自動分類（bookType）・厚さ閾値・EPUB の題名を使うかはライブラリごとに上書きできます（「StackNest 設定の値に従う」/ 個別指定）。")
