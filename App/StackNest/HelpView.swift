        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 22) {
                ...
                section("表紙を変える") {
                    ...
                    bullet("表紙を編集 — アーカイブ内の任意ページを選び、表示範囲をクロップ（ページの概念がある形式のみ。**EPUB・PDF も対象**。動画では選べません）")
                    bullet("動画からシーンを選ぶ… — mp4 / mov / m4v で、シークバー付きのプレイヤーから表紙にする場面を選ぶ。続けてその場面をクロップすることもでき、クロップせずに決定することもできます。選んだ場面は記録されるので、「表紙を再生成」で**同じ場面**を作り直せます（ローカルの本のみ。リモートでは選べません）")
