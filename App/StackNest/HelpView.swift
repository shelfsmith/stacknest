        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 22) {
                ...
                section("EPUB を読む（ベータ）") {
                    ...
                    para("EPUB の窓の操作キーは**内蔵ビューアと共通**です（設定 ▸ ビューア ▸ キー割り当て）。ページ送り・Home/End（本の最初/最後）・数字キーの位置ジャンプ・`d` 見開き切替・`+`/`-`/`=` 文字倍率・`s` 自動送り・`[`/`]` 巻移動・`f` 全画面・`?` ヘルプが効きます。ルーペやページスキップなど画像専用のものは EPUB では効きません。「全画面で開く（EPUB）」は画像ビューアとは別の設定です。")
