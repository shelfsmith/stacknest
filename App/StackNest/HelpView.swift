        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 22) {
                ...
                section("内蔵ビューア") {
                    ...
                    viewerKeyTable
                    para("見開き、ページ方向、続きから読む、巻末挙動は本ごとに永続化されます。内蔵 / 外部ビューアの切替は**画像と EPUB で別々**に持てます（設定 ▸ 表示 ▸ ビューア（画像） / ビューア（EPUB））。動画とテキスト（txt・md・rtf）は内蔵ビューアで開けないため常に外部ビューアで開きます。")
                ...
                section("外部ビューア") {
                    para("外部ビューアを使う場合は 設定 ▸ 表示（⌘,）でビューアを選びます。画像と EPUB は別々に選べます。アプリの指定は同じタブの「外部ビューア」で「選択…」からアプリ（cooViewer、Avian、プレビュー 等）を指定します。設定後はグリッドで本をダブルクリックすると外部ビューアで開きます。種類ごとに別のビューアを割り当てることもできます。動画とテキストは内蔵ビューアで開けないため、常に外部ビューアで開きます。")
                }
