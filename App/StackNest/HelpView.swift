        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 22) {
                ...
                section("リモートで使う（共有・クライアント・オフライン）") {
                    ...
                    bullet("共有する — ライブラリのツールバーのアンテナ（配信インジケータ）で共有を ON。表示される URL / QR / アクセストークンを接続側へ渡します。ロック庫は接続側でパスワード unlock が必要。")
                    bullet("Web で見る — 接続側のブラウザで共有 URL を開く（list / グリッド・全文検索・ソート・ページ送り、Web リーダーで見開き / 1 頁送り・続きから・読み方向の双方向同期）。")
                    bullet("ネイティブクライアント — タイトル画面（または File メニュー）の「サーバに接続…」で URL とトークンを入力。サイドバー / ファセット / フィルタ / 詳細のフルブラウズと内蔵ビューワで閲覧、読書進捗はサーバへ同期。**RW（編集可）トークンなら詳細ペインで編集**＝メタデータの単一 / 複数一括編集・スタンプ付与 / 定義編集・表紙ページ選択 / クロップ・読み方向（R トークンは読み取り専用）。")
