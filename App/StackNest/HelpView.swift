    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 22) {
                ...
                section("外部ビューア") {
                    para("外部ビューアを使う場合は 設定 ▸ 表示（⌘,）でビューアを選びます。画像と EPUB は別々に選べます。アプリの指定は同じタブの「外部ビューア」で「選択…」からアプリ（cooViewer、Avian、プレビュー 等）を指定します。設定後はグリッドで本をダブルクリックすると外部ビューアで開きます。種類ごとに別のビューアを割り当てることもできます。EPUB は種類としては「テキスト」に入りますが、EPUB だけ別のアプリを指定することもできます（未設定のときは「デフォルト」の設定で開きます）。動画とテキストは内蔵ビューアで開けないため、常に外部ビューアで開きます。")
                }
