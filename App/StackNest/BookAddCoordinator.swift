...
/// Adds book files (zip/cbz/cbr/rar/7z/cb7/folder) to a library bundle.
/// Each URL is parsed via the active filename_format for auto-fill, inserted into DB,
/// and its cover image is extracted into bundle/Thumbnails/<bookID>/thumbnail.jpg.
@MainActor
public final class BookAddCoordinator {
    private let database: Database
    private let bundleURL: URL
    private let format: FilenameFormat
    private static let logger = Logger(subsystem: "app.shelfsmith.stacknest", category: "BookAddCoordinator")
    ...
    public struct AddResult: @unchecked Sendable {
    ...
    public func add(urls: [URL]) async -> AddResult {
        ...
        // 自動分類設定の snapshot (loop 中で変化しない)
        let autoClassifyEnabled = ViewerSettings.shared.autoClassifyEnabled
        let thickThreshold = ViewerSettings.shared.thickBookThreshold
