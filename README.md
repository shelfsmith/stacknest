...
> **Status:** アクティブ開発中。Stackroom 互換インポート＋ブラウズ／編集／検索／内蔵ビューワ／マルチライブラリ／ロック／重複検出／ラベルカスタマイズ／DB 予防保全・修復（Phase 2.9 まで完了）が動作。**リモート共有・閲覧・編集（Phase 4）も動作**します（共有サーバ＋Web ブラウザ／Web リーダー、別 Mac からのネイティブクライアント、オフラインダウンロード。RW トークンなら、リモートからメタデータ編集・スタンプ・**表紙ページ選択／クロップ編集**も可能）。
...
- **ネイティブリモートクライアント**: 別 Mac の StackNest から別サーバへ接続して閲覧・編集（接続 / 履歴・**フルブラウズ**＝サイドバー / ファセット / フィルタ / グリッド・リスト / 詳細ペイン・ページ / 無限スクロール切替・内蔵ビューワ・進捗のサーバ同期）。**RW（編集可）トークン時は詳細ペインで編集**＝メタデータの単一 / 複数選択一括編集・スタンプ付与 / 定義編集・**表紙ページ選択 / クロップ編集**・読み方向変更（R トークンは読み取り専用）
...
- タイトル画面（または File メニュー）の **「サーバに接続…」** から URL とトークンを入力して接続します。サイドバー / ファセット / フィルタ / 詳細ペインのフルブラウズと内蔵ビューワで閲覧でき、読書進捗はサーバへ同期されます。**RW（編集可）トークンで接続した場合は、リモートのままメタデータ編集（単一 / 複数一括）・スタンプ・表紙ページ選択 / クロップ・読み方向変更ができます**（R トークンは読み取り専用）。
...
| 4.0+ | サーバー / クライアント（共有サーバ・Web ブラウザ / リーダー・ネイティブクライアント・オフライン DL・**リモート編集**［メタ / スタンプ / 表紙・クロップ］・画面サイズに応じた画像配信） | 🔄 進行中 |
