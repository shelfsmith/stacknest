...
- **ネイティブリモートクライアント**: 別 Mac の StackNest から別サーバへ接続して閲覧・編集（接続 / 履歴・**フルブラウズ**＝サイドバー / ファセット / フィルタ / グリッド・リスト / 詳細ペイン・ページ / 無限スクロール切替・内蔵ビューワ・進捗のサーバ同期）。**RW（編集可）トークン時は詳細ペインで編集**＝メタデータの単一 / 複数選択一括編集・スタンプ付与 / 定義編集・**表紙編集（アーカイブ内ページ選択 / クロップ / 外部画像を D&D・メニューで表紙に）**・読み方向変更・サーバ同期のラベルカスタマイズ（**管理者トークンならリモートで本の削除も**。R トークンでもレート / 未読は共有評価 / 閲覧状態として編集可）
...
| 4.0+ | サーバー / クライアント（共有サーバ・Web ブラウザ / リーダー・ネイティブクライアント・オフライン DL・**共有トークン**［閲覧 / 編集 / 管理者 × scope］・**リモート編集**［メタ / スタンプ / 表紙: ページ選択・クロップ・**外部画像を表紙に**／リモート追加・削除］・**リモート閲覧のディスク永続キャッシュ + 先読み + 表紙キャッシュのサーバ追従**・画面サイズに応じた画像配信・**監視フォルダ自動取込**・**ローカルアクセス（CLI `stacknest-cli` / MCP `mcp-stacknest` / OpenAPI・Redoc）**） | 🔄 進行中 |
