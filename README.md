...
既定では本は**内蔵ビューア**で開きます。内蔵 / 外部の切替は**画像と EPUB で別々**に持てます（**設定 ▸ 表示 ▸ ビューア（画像）** / **ビューア（EPUB）**）。動画とテキスト（txt・md・rtf）は内蔵ビューアで開けないため常に外部ビューアで開きます。外部ビューアを使う場合は **設定 ▸ 表示 ▸ 外部ビューア**（`⌘,`）で外部ビューアを設定してください。
