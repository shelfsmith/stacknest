...
- **ビューアキー再割当**: 設定 ▸ 内蔵ビューア で内蔵ビューアの全操作を任意キーへ再割当（競合は拒否、行ごと / 全体の既定に戻せる、ヘルプ表は現在の割当を反映）
- **大規模ライブラリ性能**: 数千〜数万件規模でソートを最適化（ICU 照合キーの前計算により、リスト更新ごとの再ソートを高速化）
- **内蔵ビューア**: 専用ウィンドウ / 全画面で、zip / cbz / cbr / 7z / フォルダ / 単一画像 / PDF を統一パイプライン表示。フィット、ピンチ / ± ズーム、ドラッグでパン、ゾーンクリック / 矢印 / Space でページ送り、数字キーで位置ジャンプ。**見開き表示**（本ごとに上書き）、本ごとのページ方向（右→左 / 左→右）、スライドショー、続きから読む、巻末挙動（停止 / 次の巻 / ループ）、前後巻ナビ、全画面で開く、**ページ送りの演出**（なし / フェード / スライド。EPUB と共通）、操作キーのフル再割当（設定 ▸ 内蔵ビューア）ほか。**インスタンス管理**で同じ本の重複起動を防止し、設定で本ごとに別ウィンドウも選べる。内蔵 / 外部ビューアは設定で切替（画像と EPUB で別々に選べる）。**ページ送りの描画は CPU アーキテクチャ別に最適化**（Apple Silicon はフル解像度の遅延デコード、Intel は背景での縮小デコード）し、アーカイブは開いたまま順方向に読み進めるため、ページ数の多い本でも引っかかりません
- **EPUB を読む（ベータ）**: テキストの EPUB は Mac では専用の窓（縦組み・ルビ・見開き、Home/End で本の最初/最後、文字倍率 `+`/`-`/`=`・位置保存、キーは内蔵ビューアと共通、配色はシステム / ライト / ダーク）。**本全体の進捗（「N / M」とバー）**を表示し、途中まで読んだ本は**「続きから / 最初から」**を訊く。**巻送りは同じ窓の中で差し替え**、画像の巻とは画像ビューアと EPUB の窓の間で切り替わる（全画面は切り替え先の設定に従う）。Web リーダーとリモート書庫（別 Mac の StackNest）でも読め、読書位置は三者で共有（Web でも画像 1 枚のページは全画面、続きから / 最初からを選べる）。全ページ画像の EPUB は画像ビューアで開く。綴じ方向は取り込み時に EPUB から本へ反映。**ベータ**: 混在本の挿絵は見開きにならない、検索・目次なし、レイアウトに違和感が出る EPUB あり
- **表紙編集**: 各本の表紙を、アーカイブ内の任意ページ選択（**EPUB・PDF も対象**）、表示範囲のクロップ、外部画像の D&D（または「外部画像を表紙に設定…」メニュー）で差し替え。**見た目（サムネイル）だけの変更で、アーカイブ本体や元ファイルは変更しません**。ローカルでもリモート（編集トークン）でも利用でき、サーバ側で表紙が変わればリモートのクライアントも一覧リロード / 再接続で追従。**表紙の再生成は 1 冊単位でも実行でき**、リンク切れの再指定（再リンク）時には表紙とページ数が自動で更新されます
...
- **対応形式**: アーカイブは ZIP / CBZ / RAR / CBR / 7z（libarchive 経由）と PDF（PDFKit）、EPUB（**ベータ**。表紙・メタデータ・閲覧。Mac では [Washi](https://github.com/shunnag/Washi) の窓で縦組み・ルビ・見開き、Web リーダーでは [foliate-js](https://github.com/johnfactotum/foliate-js)。全ページ画像の EPUB は画像ビューアで開く）、画像は JPEG / PNG / GIF / WebP / HEIC / HEIF / TIFF / AVIF（NSImage 経由）、**動画は MP4 / MOV / M4V**（表紙のみ。取り込み時に自動でフレームを拾い、右クリックからシーンを選び直せる（選んだ場面はクロップも可）。再生は外部プレイヤー。MKV / WebM / AVI は AVFoundation が開けないため表紙なし）
- **初回起動ウィザード**: 初回起動時にページ送り式ウィザードで「本の開き方（内蔵 / 外部ビューア）→（内蔵選択時）ビューア初期設定 → 最初のライブラリ（新規 / 開く / 取り込み）」を設定。設定 ▸ 一般 からいつでも再表示可能
- **インポート**: 既存 Stackroom ライブラリ XML から SQLite DB へ移行
- **監視フォルダ（自動取込）**: 指定フォルダを監視し、置かれたアーカイブ / 画像フォルダを自動でライブラリへ取り込み（フォルダごとの命名プリセット、初回プレビュー、取込要約バナー）。自動分類（bookType）・厚さ閾値・EPUB の題名を使うかはライブラリごとに上書き可能（「StackNest 設定の値に従う」/ 個別指定）。サブフォルダの扱いはフォルダごとに **3 択**（取り込まない / 1 冊として取り込む / 中も個別に取り込む）
...
- **オフラインダウンロード**: リモートの本を選んでローカル保存し、接続なしで一覧と閲覧（続きから）。外部ビューアを選んでいる形式は外部ビューアで開く（動画も。題名のファイル名で渡す）。タイトル画面 / File メニューの「オフライン」から起動。複数選択で一括ダウンロード / 一括削除（リモート庫のグリッド / 一覧の右クリック「オフラインから削除」でも可）
...
**初回起動時**（新規インストールで設定なし）は初回起動ウィザードが表示され、本の開き方と最初のライブラリを順に設定します。2 回目以降やライブラリ未選択時はタイトル画面が表示され、次の 3 つの選択肢を提示します：
...
既定では本は**内蔵ビューア**で開きます。内蔵 / 外部の切替は**画像と EPUB で別々**に持てます（**設定 ▸ 表示 ▸ ビューア**）。動画とテキスト（txt・md・rtf）は内蔵ビューアで開けないため常に外部ビューアで開きます。外部ビューアを使う場合は **設定 ▸ 表示 ▸ 外部ビューア**（`⌘,`）で外部ビューアを設定してください。
「選択…」をクリックして画像ビューアアプリ（cooViewer、Avian、プレビュー 等）を選んでください。種類（画像 / テキスト / 動画）ごとに別のアプリを割り当てられ、EPUB だけさらに別のアプリを指定することもできます（未設定ならデフォルトの設定で開きます）。
...
直接選択した外部ビューアで開きます。オフライン（ダウンロード済み）の本も同じ設定に従い、外部ビューアには本の題名のファイル名で渡します。
...
**Web ブラウザから見る**
- 接続側のブラウザで共有 URL を開くと、list / grid、全文検索、ソート、ページ送りでブラウズでき、本を開くと Web リーダー（先読み、見開き / 1 頁送り、続きから、読み方向同期）で読めます。テキストの EPUB も読め、画像 1 枚のページは全画面で表示し、本の詳細で「続きから / 最初から」を選べます。
...
- タイトル画面 / File メニューの **「オフライン（ダウンロード済み）」** から、**サーバに接続していなくても**ダウンロード済みの本を一覧と閲覧ができます（続きから、巻送り対応。外部ビューアを選んでいる形式は外部ビューアで開きます）。不要になった本は「選択」モードで一括削除できます（リモート庫のグリッド / 一覧の右クリック「オフラインから削除」でも消せます）。
...
| 4.8 | **EPUB の操作キーを内蔵ビューアと共通化**（同じ割り当て表・再割当が EPUB にも効く／`Home`・`End` が本の最初と最後へ／割合ジャンプ・見開き切替・文字倍率・全画面・自動送り・巻移動／EPUB 専用の全画面設定）、**オフライン保存のファイル名を元の拡張子に**（起動時に既存分も補正）、Washi 1.17.0 — v0.14.4 | ✅ 完了 |
| 4.9 | **EPUB を読みやすく**（本全体の進捗「N / M」とバー・「続きから / 最初から」・巻送りを同じ窓で差し替え・画像ビューアとの切り替え・配色）、**ページ送りの演出**（なし / フェード / スライド・両ビューア共通）、**EPUB 用の外部ビューア**とオフラインの外部ビューア対応、設定の「内蔵ビューア」タブ、表紙編集の入口（EPUB・PDF・動画のシーンの切り抜き）と EPUB の題名を取り込みに使う設定、Web リーダーのテキスト EPUB（画像ページを全画面・続きから / 最初から）、Washi 1.21.0 — v0.15.0 | ✅ 完了 |
